document.addEventListener("DOMContentLoaded", ()=>{
  ...
  cart.forEach((item) => displayCart(item));
   //counter variable for checkout total
  ...
  products.forEach((product) => {
    ...
    //adding the product to the cart array and displaying cart
    li.querySelector("button").addEventListener("click", () => {
      cart.push(product);
      saveCart();
      displayCart(product); //to display cart products
    });
  ...
  function displayCart(product) {
    ...
    cartItem.querySelector(`button`).addEventListener("click", () => {
      ...
      saveCart();
      let calculatedPrice = Math.round((ctr - product.price) * 100) / 100;
      totalPrice.innerHTML = `${calculatedPrice}`;
      ctr = ctr - product.price;
    });
  }
